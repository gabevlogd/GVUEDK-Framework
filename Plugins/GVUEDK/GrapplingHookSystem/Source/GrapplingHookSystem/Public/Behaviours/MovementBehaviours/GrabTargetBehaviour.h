...
#include "GrabTargetBehaviour.generated.h"
 ...
 *
 */
UCLASS(meta = (DisplayName = "GHGrabTargetBehaviour"))
class GRAPPLINGHOOKSYSTEM_API UGrabTargetBehaviour : public UGrapplingHookMovementBehaviour
{
	GENERATED_BODY()

public:
	UGrabTargetBehaviour();
...
private:
	...
	/**
	 * Collision channel for the trace performed to find if the hook hit an obstacle during the way back motion.
	 */
	UPROPERTY(EditAnywhere, meta = (AllowPrivateAccess = "true"))
	TEnumAsByte<ECollisionChannel> StopHookTraceChannel;
	...
	/**
	 * Distance from the target at which the hook should stop the way back motion and release the target.
	 */
	UPROPERTY(EditAnywhere, meta = (AllowPrivateAccess = "true", ClampMin = 0.f))
	float StopGrabDistance = 0.f;
	...
	FVector HookStartSocketRelativeLocation;
